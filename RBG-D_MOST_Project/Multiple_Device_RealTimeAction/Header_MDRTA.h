...
//shijian-> 1:停止 2走 3坐起來 4站起來 5跌倒 6坐下 7躺下 
int eventDetect(cv::Point3f head_Pre, cv::Point3f torsol_Pre, cv::Point3f head, cv::Point3f torsol, cv::Point3f head_Ini, cv::Point3f torsol_Ini, cv::Point3f neckkk,int fallDownThreshold) //(前一幀,後一幀,初始幀)
{
...
//顯示出現在狀態
void displayState(int nextState, int nowState,int deviceNum=-1){
	if (deviceNum >= 0){
		if (nextState != nowState)
		{
			if (nextState == 1)
				cout << deviceNum << "目前狀態: " << "站著" << endl;
			if (nextState == 2)
				cout << deviceNum << "目前狀態: " << "走著" << endl;
			if (nextState == 3)
				cout << deviceNum << "目前狀態: " << "跌倒" << endl;
			if (nextState == 4)
				cout << deviceNum << "目前狀態: " << "坐著" << endl;
			if (nextState == 5)
				cout << deviceNum << "目前狀態: " << "躺著" << endl;
		}
	}
	else{
		if (nextState != nowState)
...
#ifndef MultipleDeviceClass
#define MultipleDeviceClass 

class CDevice{
private:
	static int DeviceNumbers ;//計算總共有幾台裝置

public:
    int deviceID;//當下裝置ID
	string sWindow;
	openni::Device* pDevice;
	UserTracker* pUserTracker;

	int fallDownThreshold = 700; //跌倒的閥值 應該會被蓋掉
	...
	CDevice(int idx, const char* uri){ //機器編號,機器URI
		DeviceNumbers++;//裝置數量+1
		deviceID = DeviceNumbers;//賦予裝置編號
		pDevice = new openni::Device();
		pDevice->open(uri);
		pUserTracker = new UserTracker();

		if (pUserTracker->create(pDevice) != STATUS_OK){
			cerr << "Can't create user tracker" << endl;
		}
		pUserTracker->setSkeletonSmoothingFactor(0.8);

		stringstream mStrStream;//此function 使用到imgproc.hpp 
		mStrStream << "Sensor_" << idx << endl;
		sWindow = mStrStream.str();
		cv::namedWindow(sWindow.c_str(), CV_WINDOW_AUTOSIZE);
	}
};
int CDevice::DeviceNumbers = 0;
#endif
